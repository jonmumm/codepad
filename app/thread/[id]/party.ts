import { json, notFound, ok } from "@/party/utils/response";
 ...
 */
export default class ThreadServer implements Party.Server {
  //   messages?: Message[];
  //   botId?: string;
  ...
  async onRequest(request: Party.Request) {
    ...
    if (request.method === "POST") {
      ...
      const { id, systemPrompt } = CreateThreadPropsSchema.parse(
        await request.json()
      );
      await this.party.storage.put("systemPrompt", systemPrompt);
      await this.party.storage.put("id", id);
      return ok();
    }
...
ThreadServer satisfies Party.Worker;
